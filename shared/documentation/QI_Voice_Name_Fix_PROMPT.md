...
- Reference implementation that already does this correctly: **`C:\CLAUDE\Claude Voice`** (`speak.py`, `realtime.py`, `telegram_bot.py`, `pronounce.py`).
...
- **AvatarStudio** — `C:\1-AI\APPS\AvatarStudio`. TTS call sites: `avatar_pipeline.py`, `avatar_studio.py`, `gen_voices_kokoro.py` (Kokoro), `gen_tokyo_times_tts.py`, `test_edge_tts.py`. Apply the fix wherever a script/line is sent to edge-tts **or Kokoro** before render/lip-sync.
- **Claude Voice** — `C:\CLAUDE\Claude Voice` (already fixed; use as the model).
...
- Reference app that's already correct: `C:\CLAUDE\Claude Voice`.
- To teach a new word/name, edit the `map` in `C:\QIH\shared\voice\pronunciation.json`.
- First target to fix with this prompt: **AvatarStudio** (`C:\1-AI\APPS\AvatarStudio`).
