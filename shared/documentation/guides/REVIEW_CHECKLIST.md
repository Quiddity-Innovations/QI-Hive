...
- ☐ [lambda_function.py](C:\QI\TOOLS\aws_relay\lambda_function.py) — signature verification line by line
- ☐ [deploy_lambda.py](C:\QI\TOOLS\aws_relay\deploy_lambda.py) — idempotent deploy pattern + the dual-permission fix
- ☐ [queue_drainer.py](C:\QI\TOOLS\aws_relay\queue_drainer.py) — long-poll, verdicts, poison handling
...
- ☐ [Dockerfile](C:\QI\TOOLS\aws_relay\Dockerfile) + [docker-compose.yml](C:\QI\TOOLS\aws_relay\docker-compose.yml) (drainer + monitor sidecar)
- ☐ [queue_monitor.py](C:\QI\TOOLS\aws_relay\queue_monitor.py) — the C2 sidecar & its ALERT logic
- ☐ [K8s_Foundations_Guide.md](K8s_Foundations_Guide.md) — k3s, self-healing demo, parking pattern, gotchas
- ☐ [k8s/qi-relay.yaml](C:\QI\TOOLS\aws_relay\k8s\qi-relay.yaml) — Deployment + CronJob anatomy
- ☐ [helm/qi-bot chart](C:\QI\TOOLS\aws_relay\helm\qi-bot\) — chart vs values; `maia` + `demobot` releases = the template engine
- ☐ Check `C:\QI\LOGS\queue_report_k8s.log` — has the CronJob been firing every 15 min?
...
- ☐ [providers.py](C:\QI\TOOLS\aws_relay\cloud_brain\providers.py) — the adapter pattern, rung by rung
- ☐ [brain_lambda.py](C:\QI\TOOLS\aws_relay\cloud_brain\brain_lambda.py) — wake-on-stale-queue architecture
