...
Gateway: `http://127.0.0.1:8651` (Windows service `MapSnapBU_MCP`; health = `GET /health` → `{"status":"ok"}`). It proxies `MapSnapBU_Web` on `:9876`. Tokens in `C:/APPS/MapSnap/config/secrets/` — `mcp_bearer_token.txt` (Claude Code, `Authorization: Bearer`) and `mcp_path_token.txt` (Desktop, rides in the URL as `/c/<token>/mcp`). Never print a token.

**Why it looks absent in a BU Hive session:** `mapsnap-bu` is registered in `~/.claude.json` **project-scoped to `C:/APPS/MapSnap` only**, so it does not bind when cwd is `C:/AI/BU Hive`, and it is **not** a plugin (`~/.claude/plugins/repos` is empty). Grepping the BU Hive repo finds nothing and proves nothing. Reach it anyway with direct loopback `POST /mcp` + the bearer header. The user does **not** want mapsnap registered inside BU Hive — do not `claude mcp add` it there.
...
**These environments are Hyland Cloud — no SQL engine.** The schema lists ~11,450 tables but only 7 are readable live (`LIVE_TABLE_MAP` in `C:/APPS/MapSnap/Application/onbase_unity.py`): `customquery`, `filetype`, `itemtype`, `itemtypegroup`, `keytypetable`, `notetype`, `usergroup`. Anything else returns `unavailable` + HTTP 404 — that is by design, **not** a permission block and not a broken service.
