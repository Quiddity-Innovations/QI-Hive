...
| 8600 | Claude Manager Dashboard | `C:\CLAUDE\Dashboard\` | 🤔 Decide | Meant for Claude-agent orchestration. 9000 already has an "Agent Delegations" tab. Either: (a) merge into 9000, or (b) keep as the "internal" agent view and iframe it into 9000. |
...
### `C:\QI` (Maia) — ⚠️ DO NOT COMMIT BEFORE DEMO
Modified files:
...
### `C:\NEXUS` — ⚠️ DO NOT COMMIT BEFORE DEMO
...
- `_build_easyflow_strategy_docx.py` is a build script in the summaries folder — should live in `C:\EasyFlow\TOOLS\`.
...
Also: Claude Manager has its own `C:\CLAUDE\Session Summaries\` folder. **Decision needed:** collapse into the universal folder, or keep Claude Manager separate?

---

## 🟣 MCP instability (from `status.json` pending actions)

Per `C:\CLAUDE\status.json`:
...
5. Move `_build_easyflow_strategy_docx.py` out of `Session_Summaries\` into `C:\EasyFlow\TOOLS\`.
