...
**Current path:** `C:\APPS\OC` · **Target path:** `C:\APPS\QIP\OpenClaw`
...
| Surface | Where | What points at `C:\APPS\OC` |
|---|---|---|
| **NSSM service** | `OC-Keepalive-Service` | `AppDirectory=C:\APPS\OC\tools`, `AppParameters=C:\APPS\OC\tools\oc-keepalive-daemon.py`, `AppStdout/Stderr=C:\APPS\OC\runtime\logs\keepalive\*.log` |
| **Task Scheduler** | `OC_WSL_KeepAlive` | Runs `powershell -File "C:\APPS\OC\keep-wsl-alive.ps1"` |
| **Task Scheduler** | `OC-ChatGPT-Keepalive` | Args `C:\APPS\OC\tools\oc-chatgpt-keepalive.py`, WorkingDirectory `C:\APPS\OC\tools` |
| **Task Scheduler** | Ubuntu yubin-daily task | `bash /mnt/c/OC/repo/scripts/yubin/yubin-daily.sh` |
| **PowerShell script** | `C:\APPS\OC\keep-wsl-alive.ps1` | `$LOG_DIR = "C:\APPS\OC\runtime\logs"` + `exec /mnt/c/OC/oc-watchdog.sh` |
| **Bash watchdog** | `C:\APPS\OC\oc-watchdog.sh` | `LOG=/mnt/c/OC/runtime/logs/wsl-watchdog.log` |
| **Python daemons** | `C:\APPS\OC\tools\*.py` | Multiple `Path(r"C:\APPS\OC\runtime\...")` |
...
- [ ] Stand up empty `C:\APPS\QIP\OpenClaw\` with QI 7-folder layout (`engine/`, `config/`, `data/`, `logs/`, `tests/`, `docs/`, `tools/`)
- [ ] Decide which existing `C:\APPS\OC\` subdirs map to which QI folders:
  - `C:\APPS\OC\tools\` → `C:\APPS\QIP\OpenClaw\tools\`
  - `C:\APPS\OC\repo\` → keep as submodule reference in engine/ or top-level
  - `C:\APPS\OC\runtime\` → `C:\APPS\QIP\OpenClaw\data\` + `logs\`
  - `C:\APPS\OC\Documentation\` → `C:\APPS\QIP\OpenClaw\docs\`
  - `C:\APPS\OC\secrets\` → **do not move; relocate to password manager + reference via env vars**
- [ ] Audit all `.py`, `.sh`, `.ps1`, `.bat` for hardcoded `C:\APPS\OC` → produce sed/replace script
...
- [ ] `robocopy C:\APPS\OC C:\APPS\QIP\OpenClaw /E /COPYALL` (excluding runtime/, __pycache__)
...
- [ ] Reconfigure NSSM service via broker:
  - `nssm set OC-Keepalive-Service AppDirectory C:\APPS\QIP\OpenClaw\tools`
  - `nssm set OC-Keepalive-Service AppParameters C:\APPS\QIP\OpenClaw\tools\oc-keepalive-daemon.py`
...
- [ ] Watch `C:\APPS\QIP\OpenClaw\logs\` for clean startup of all daemons
...
- [ ] Remove or archive `C:\APPS\OC\` after 48h of stable operation on new path
...
- [ ] Add `nssm install` rule to elevation broker whitelist (scoped to `QI_*` names + paths under `C:\QIH|C:\APPS\QIP`) so future service creations don't need a manual admin bat
- [ ] Initialize or link the OpenClaw git repo to a private GitHub remote

## What the 2026-04-19 safe-tier onboarding accomplished

- ✅ `git init` in `C:\APPS\OC` → future silent regressions (like the CRLF bug that ran for 11 days undetected) will surface in diffs
...
The following plaintext credential files currently sit in `C:\APPS\OC\` root and are excluded from git but still present on disk. They should be moved to a password manager:
