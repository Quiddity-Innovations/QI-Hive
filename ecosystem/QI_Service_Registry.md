...
### QI_M2VTunnel — REMOVED 2026-09-10 (M2V retired by owner decision; config + creds archived in D:\Dev\_archive\M2V_2026-09-10\tunnel; Cloudflare-side tunnel/DNS for m2v.quiddityinnovations.com still to be deleted by Renne)
| Field | Value |
|---|---|
...
| **⏸ OFF 2026-08-20 → 2026-09-19** | Whole stack deliberately powered down for a troubleshooting window at Renne's request. All four services are **STOPPED and pinned to `SERVICE_DEMAND_START`** — the pin is the point: they are AUTO_START normally, so a reboot during the window would silently revive the public LINE and Telegram bots. `claudevoice.quiddityinnovations.com` is **offline** until restore. Power switch: `python C:\QIH\tools\claudevoice_power.py off\|on\|status` (brokered through QI_Elevate, needs no admin shell). Auto-restore: scheduled task `QI_ClaudeVoiceRestore_20260919`. To bring it back early, run the `on` command. **Do not drive this stack from the Hive dashboard's Ops launcher** — its writes 403 through the tunnel (`tunnel_write_guard`, `engine/hive/dashboard/server.py:57`) and its desktop actions spawn into Session 0, so it reports success while doing nothing visible. Sturdy path instead: **`C:\APPS\CLAUDE\Claude Voice\ClaudeVoice_Control.bat`** — rewritten 2026-08-20 into one menu (status / on / off / :8720 brain UI / :8724 cockpit) and also usable non-interactively as `ClaudeVoice_Control.bat status\|on\|off`. |
| **⚠ Why "off" would not hold (root cause, 2026-08-20)** | Stopping the services does **not** stop Claude Voice. The stack registers a **global Claude Code `SessionStart` hook** in `C:\Users\renne\.claude\settings.json` → `session_hook.py start`, which starts `session_watch.py`; that supervisor then auto-arms on sight of any open Claude session and launches `bridge_responder.py` + `realtime.py`. Because the hook is user-level, **every new Claude Code session in every project re-armed the voice stack within ~20s of it being killed** — observed repeatedly (15:28, 15:43) with the session count climbing 4→5→7. A `Stop` hook (`speak_response.py`) likewise speaks replies aloud. **Fix:** kill-switch file `C:\APPS\CLAUDE\Claude Voice\data\VOICE_DISABLED`; both `session_hook.py` and `speak_response.py` check it and no-op while it exists. `claudevoice_power.py off` writes it **before** killing anything (so a session opening mid-shutdown can't re-arm behind you); `on` removes it. If Claude Voice ever "won't stay dead", check this flag first. |
...
| Field | Value |
|---|---|
| **QI_AvatarStudio** | `C:\APPS\AvatarStudio\.venv\Scripts\python.exe engine\service.py` |
| AppDirectory | `C:\APPS\AvatarStudio` |
| Description | QI Avatar Studio - talking-head video pipeline :7862. FastAPI plug-in service (health/version/info + /api/render job API) serving the Gradio panel at / with ?embed=1. WSL2 render backends: Hallo2, LivePortrait, video-retalking. |
| Port | 7862 (loopback only - never tunnel; /api/render reads local portrait and wav paths) |
